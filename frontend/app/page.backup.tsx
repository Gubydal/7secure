...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#09090b] text-[#fafafa] font-sans">
      {/* Navigation */}
      <header className="flex items-center justify-between px-6 py-4 border-b border-white/10 sticky top-0 bg-[#09090b]/80 backdrop-blur-md z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="primary" className="bg-white text-black font-semibold text-sm h-9 px-4 rounded-md hover:bg-zinc-200 transition-colors">
            Subscribe
          </Button>
        </div>
      </header>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-6 py-16 flex flex-col items-center">
        {/* Hero Section */}
        <section className="flex flex-col items-center text-center max-w-3xl mb-24 mt-10">
          <Image
            src="/Small_Icon.svg"
            alt="7secure icon"
            width={48}
          ...
          <div className="w-full max-w-md flex items-center bg-zinc-900 border border-white/10 p-1.5 rounded-xl transition focus-within:border-white/30">
            <input 
              ...
              className="flex-1 bg-transparent border-none outline-none text-white px-4 placeholder-zinc-500"
            />
            <Button variant="primary" className="bg-white text-black font-semibold h-11 px-6 rounded-lg ml-2 hover:bg-zinc-200 transition-colors">
              Subscribe
            </Button>
          </div>
          ...
          <div className="mt-16 flex flex-col items-center">
            ...
            <div className="flex flex-wrap justify-center items-center gap-8 opacity-60 grayscale">
              ...
              <span className="text-xl font-bold font-mono">CLOUDFLARE</span>
              <span className="text-xl font-bold font-mono">PALO ALTO</span>
              <span className="text-xl font-bold font-mono">CROWDSTRIKE</span>
            </div>
          </div>
        </section>
        ...
        <section className="w-full mb-20 text-left">
          <div className="border-b border-white/10 pb-4 mb-8">
            <h2 className="text-3xl font-bold text-white mb-2">Latest Intel</h2>
            <p className="text-zinc-400">The latest developments in InfoSec and Threat Intelligence.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             {articles.map((article, i) => (
                <a href="#" key={i} className="group block bg-zinc-900/50 border border-white/10 rounded-2xl p-6 hover:bg-zinc-900 hover:border-white/20 transition-all duration-200">
                  <div className="aspect-[16/9] w-full bg-zinc-800 rounded-lg mb-6 overflow-hidden relative">
                    <div className="absolute inset-0 bg-gradient-to-br from-zinc-700 to-zinc-900 opacity-50 group-hover:opacity-75 transition-opacity"></div>
                    <div className="absolute bottom-4 left-4 text-xs font-mono text-white/50 bg-black/50 px-2 py-1 rounded backdrop-blur-sm">7SECURE BRIEF</div>
                  </div>
                  <p className="text-sm text-zinc-500 font-mono mb-3">{article.label}</p>
                  <h3 className="text-xl font-semibold text-white mb-3 group-hover:text-blue-400 transition-colors">{article.title}</h3>
          ...
          <div className="mt-10 text-center">
             <Button variant="outline" className="border-white/20 text-white hover:bg-white/5 bg-transparent font-medium h-10 px-6 rounded-md">
               View All
             </Button>
          </div>
        </section>
        ...
        <section className="w-full mb-16 text-left">
          <div className="border-b border-white/10 pb-4 mb-8">
            <h2 className="text-3xl font-bold text-white mb-2">Deep Dives</h2>
            <p className="text-zinc-400">In-depth technical writeups and strategic security frameworks.</p>
          </div>
          <div className="flex flex-col gap-4">
             {["Building a modern SOC from scratch", "Deconstructing the latest APT29 Campaign", "Automating malware analysis with AI"].map((exclusive, i) => (
               <a href="#" key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-5 rounded-xl border border-white/5 bg-zinc-900/30 hover:bg-zinc-900 hover:border-white/10 transition-all group gap-4">
                 <div>
                   <h3 className="text-lg font-semibold text-white group-hover:text-blue-400 transition-colors mb-1">{exclusive}</h3>
                   <p className="text-sm text-zinc-500">Exclusive Technical Series</p>
                 </div>
      ...
      <footer className="border-t border-white/10 mt-auto py-12 px-6">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <p className="text-sm text-zinc-600">© 2026 7secure Inc. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
